...
def BetterSpecAnal(x):
    ...
    # Starting points
    start_row = (rows - (5 * 64)) // 2
    start_col = (cols - (5 * 64)) // 2

    # 2D Hamming window
    W = np.outer(np.hamming(64), np.hamming(64))

    better_psd = np.zeros((64, 64))
    for i in range(5):
        for j in range(5):
            # Extract window
            row_start = start_row + i * 64
            col_start = start_col + j * 64
            window = x[row_start:row_start + 64, col_start:col_start + 64]

            # Apply Hamming window
            window = window * W

            # Compute squared DFT magnitude
            window = (1/64**2)*np.abs(np.fft.fft2(window))**2
            window = np.fft.fftshift(window)

            # Sum the averaged window
            better_psd += window/25
